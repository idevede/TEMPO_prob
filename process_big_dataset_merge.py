...
def merge_arrow_files():
    # 创建目标目录
    target_dir = "dataset/chronos_0.01_together/arrow_files"
    os.makedirs(target_dir, exist_ok=True)
    
    # 获取所有arrow文件
    arrow_files = glob.glob("dataset/chronos_0.01/**/arrow_files/*.arrow", recursive=True)
    ...
    total_samples = 0
    dataset_info_files = glob.glob("dataset/chronos_0.01/**/dataset_files/dataset_info.json", recursive=True)
    ...
    target_dir = "dataset/chronos_0.01_together/dataset_files"
    os.makedirs(target_dir, exist_ok=True)
    # 创建新的dataset_info文件
    new_info = {
